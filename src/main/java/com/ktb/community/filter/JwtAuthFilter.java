...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    // 필터 제외 경로 목록
    private static final List<String> excludePathPatterns = new ArrayList<>(List.of("/api/auth", "/api/users","/swagger-ui", "/v3/api-docs", "/error"));

    // 필터 제외 경로 설정
    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        String path = request.getRequestURI();
        return Arrays.stream(excludePathPatterns.toArray(new String[0])).anyMatch(path::matches);
    }
